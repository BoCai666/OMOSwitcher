...
import { addCustomProvider } from '@/services/opencodeModels'
...
const props = defineProps<{
  visible: boolean
  /** 已有供应商列表，用于 ID 去重校验 */
  existingProviders: ProviderWithAvailability[]
}>()
...
function validateAddForm(): string | null {
  ...
  // 检查是否与已有供应商冲突
  if (props.existingProviders.some(p => p.id === form.providerId.trim())) {
